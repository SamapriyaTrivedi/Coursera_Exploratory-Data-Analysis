################### Importing Data ################### 
setwd("C:/Users/Samapriya Trivedi/Coursera Exploratory Data Analysis")

data_full <- read.csv("household_power_consumption.txt", header=T, sep=';', na.strings="?", 
                      nrows=2075259, check.names=F, stringsAsFactors=F, comment.char="", quote='\"')


################### PLOT 1 ################### 
data1 <- subset(data_full, Date %in% c("1/2/2007","2/2/2007"))
data1$Date <- as.Date(data1$Date, format="%d/%m/%Y")
hist(data1$Global_active_power, main="Global Active Power", 
     xlab="Global Active Power (kilowatts)", ylab="Frequency", col="Red")

png("Plot 1.png", width=480, height=480)
dev.off()


################### PLOT 2 ###################
data2 <- subset(data_full, Date %in% c("1/2/2007","2/2/2007"))
data2$Date <- as.Date(data2$Date, format="%d/%m/%Y")
datetime <- paste(as.Date(data2$Date), data2$Time)
data2$Datetime <- as.POSIXct(datetime)

with(data2, {plot(Global_active_power~Datetime, type="l",
                  ylab="Global Active Power (kilowatts)", xlab="")})

dev.copy(png, file="Plot 2.png", height=480, width=480)
dev.off()


################### PLOT 3 ###################
data3 <- subset(data_full, Date %in% c("1/2/2007","2/2/2007"))
data3$Date <- as.Date(data3$Date, format="%d/%m/%Y")
datetime <- paste(as.Date(data3$Date), data3$Time)
data3$Datetime <- as.POSIXct(datetime)

with(data1, {plot(Sub_metering_1~Datetime, type="l",
                  ylab="Global Active Power (kilowatts)", xlab="")
  lines(Sub_metering_2~Datetime,col='Red')
  lines(Sub_metering_3~Datetime,col='Blue')})

legend("topright", col=c("black", "red", "blue"), lty=1, lwd=2,
       legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

dev.copy(png, file="Plot 3.png", height=480, width=480)
dev.off()

################### PLOT 4 ###################
data4 <- subset(data_full, Date %in% c("1/2/2007","2/2/2007"))
data4$Date <- as.Date(data4$Date, format="%d/%m/%Y")
datetime <- paste(as.Date(data4$Date), data4$Time)
data4$Datetime <- as.POSIXct(datetime)

par(mfrow=c(2,2), mar=c(4,4,2,1), oma=c(0,0,2,0))

with(data4, {plot(Global_active_power~Datetime, type="l", 
       ylab="Global Active Power (kilowatts)", xlab="")
  plot(Voltage~Datetime, type="l", 
       ylab="Voltage (volt)", xlab="")
  plot(Sub_metering_1~Datetime, type="l", 
       ylab="Global Active Power (kilowatts)", xlab="")
  lines(Sub_metering_2~Datetime,col='Red')
  lines(Sub_metering_3~Datetime,col='Blue')
  legend("topright", col=c("black", "red", "blue"), lty=1, lwd=2, bty="n",
         legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
  plot(Global_reactive_power~Datetime, type="l", 
       ylab="Global Rective Power (kilowatts)",xlab="")
})

dev.copy(png, file="Plot 4.png", height=480, width=480)
dev.off()